...
function getVehicleFillInfo(vehicle)
    ...
    -- Sicherheitsnetz: Falls kein Heckstreuer gefunden wurde, Hauptfahrzeug prüfen
...
function WinterdienstMission:update(dt)
    ...
    elseif self.state == STATE_DRIVING_ROUTE then
        ...
        if vehicle ~= nil then
            ...
            -- REPARATUR: Wir zählen jetzt in echten Sekunden-Schritten (3 Ticks = 3 Sekunden)
            if self.adStartTimer == nil then
                self.adStartTimer = 3
                debugLog("[Hydraulik-Wartezeit] Startpunkt erreicht. Warte 3 Sekunden auf Schild-Animation...")
            end
            
            if self.adStartTimer > 0 then
                self.adStartTimer = self.adStartTimer - 1 -- Zieht pro Sekunde exakt 1 ab!
                if vehicle.ad ~= nil and vehicle.ad.driveModule ~= nil then
                    vehicle.ad.driveModule.isActive = false -- AutoDrive im Stand pausieren
                end
                debugLog(string.format("[Hydraulik-Wartezeit] Schild senkt sich... Noch %d Sekunden Pause.", self.adStartTimer))
            else
                -- Die 3 Sekunden sind um! Jetzt wecken wir AutoDrive unbarmherzig auf!
                if vehicle.ad ~= nil and vehicle.ad.driveModule ~= nil then
                ...
                if vehicle.setCruiseControlState ~= nil then 
                    pcall(vehicle.setCruiseControlState, vehicle, 1) 
                end
                
                -- Auswertung der Fahrt starten, sobald er rollt
                self:checkArrival()
            end
        else
            self:checkArrival()
...
-- LS25-Standard-Funktionen für globale Skripte
function WinterdienstMission:loadMap(name)
    debugLog("System initialisiert - Warte auf Schnee im Januar...")
end

function WinterdienstMission:draw()
    -- Erzeugt die fahrzeugunabhängige Tastenprüfung in jedem Frame
    if self.updateActionEvent ~= nil then
        self:updateActionEvent()
    end
end

function WinterdienstMission:mouseEvent(posX, posY, isDown, isUp, button) end
function WinterdienstMission:keyEvent(uniqKey, sym, modifier, isDown) end

local winterdienstMissionInstance = nil

local function initWinterdienstMission(mission)
    if winterdienstMissionInstance == nil then
        winterdienstMissionInstance = WinterdienstMission.new()
        
        -- Das ist der korrekte LS25-Befehl, der den Absturz verhindert!
        ...
        print("[Winterdienst] Mod erfolgreich geladen und im LS25-System registriert!")
    end
end
